...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  const filteredAttendance = useMemo(() => {
    ...
    if (currentUser.role === "super_admin") {
      if (state.selectedSchoolId) {
        // ⚡ Bolt Performance Optimization: Use O(1) Set lookup instead of O(N*M) array finds
        const validPupilIds = new Set(
          state.pupils.filter((p) => p.schoolId === state.selectedSchoolId).map((p) => p.id),
        );
    ...
    // ⚡ Bolt Performance Optimization: Use O(1) Set lookup instead of O(N*M) array finds
    const validPupilIds = new Set(
      state.pupils.filter((p) => p.schoolId === currentUser.schoolId).map((p) => p.id),
    );
    return state.attendance.filter((a) => validPupilIds.has(a.pupilId));
  ...
  const filteredAudit = useMemo(() => {
    ...
    if (currentUser.role === "super_admin") {
      if (state.selectedSchoolId) {
        ...
        const validUserIds = new Set(
          state.users.filter((u) => u.schoolId === state.selectedSchoolId).map((u) => u.id),
        );
    ...
    // ⚡ Bolt Performance Optimization: Use O(1) Set lookup instead of O(N*M) array finds
    const validUserIds = new Set(
      state.users.filter((u) => u.schoolId === currentUser.schoolId).map((u) => u.id),
    );
    return nonCacheAudit.filter((a) => validUserIds.has(a.actorId));

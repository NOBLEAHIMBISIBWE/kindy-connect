...
const outstandingAmount = (amountDue: number, amountPaid: number) =>
  Math.max(0, (Number(amountDue) || 0) - (Number(amountPaid) || 0));
...
function Summary({ title, value, tone = "" }: { title: string; value: number; tone?: string }) { return <Card className="border-0 shadow-sm"><CardContent className="p-5"><p className="text-sm text-muted-foreground">{title}</p><p className={`mt-1 text-2xl font-semibold ${tone}`}>{(Number(value) || 0).toFixed(2)}</p></CardContent></Card>; }
